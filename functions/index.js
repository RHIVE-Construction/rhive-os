 ...
 */
exports.justCallWebhook = functions.runWith({ secrets: ['JUSTCALL_API_KEY', 'JUSTCALL_API_SECRET', 'JUSTCALL_FROM_NUMBER'] }).https.onRequest((req, res) => {
    return cors(req, res, async () => {
        if (req.method !== 'POST') {
        ...
        // Handle CTA Lead Notifications
        if (eventType === 'client.cta_lead') {
            const { address, phone, email, concern } = body.data || {};
            if (!phone) {
            ...
            try {
                // Format the SMS
                const smsMessage = `New RHIVE CTA Lead Inquiry:\nConcern: ${concern || 'General'}\nAddress: ${address || 'Not Provided'}\nPhone: ${phone}\nEmail: ${email || 'Not Provided'}`;
                ...
                if (apiKey && apiSecret && fromNumber) {
                    ...
                    if (response.status >= 200 && response.status < 300) {
                        smsSent = true;
                        console.log('[justCallWebhook] JustCall SMS sent successfully:', response.data);
                    } else {
                        smsError = response.data?.message || JSON.stringify(response.data);
                        console.error('[justCallWebhook] JustCall error:', response.data);
                    }
                } else {
                    console.log(`\n==================================================`);
                    console.log(`[SMS SIMULATION] To: +17438876637`);
                ...
                // Save to Firestore
                await db.collection('cta_leads').add({
                    address: address || '',
                    phone,
                    email: email || '',
                    concern: concern || 'General',
                ...
                if (!smsSent) {
                    return res.status(502).json({ error: `Failed to send SMS: ${smsError}` });
                }

                return res.status(200).json({ success: true, message: 'Notification sent successfully.' });
            } catch (err) {
                console.error('Error handling CTA lead:', err);
