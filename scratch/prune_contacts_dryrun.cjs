...
async function run() {
    console.log("📥 Fetching Google Sheet contacts via Axios...");
    ...
    // If the first row is metadata, find the actual header
    ...
    snapshot.forEach(doc => {
        ...
        for (const sc of sheetContacts) {
            // 1. Match by Email (if valid and not a justcall dummy email)
            ...
            // 2. Match by Phone (if valid and matching last 10 digits)
            ...
            // 3. Match by Name (exact normalized match)
    ...
    console.log("\n=== DRY RUN RESULTS ===");
    console.log(`Total Firestore Contacts: ${snapshot.size}`);
    console.log(`Retained Contacts: ${retainList.length}`);
    console.log(`Deleted Contacts: ${deleteList.length}`);
    console.log("========================\n");

    // Print a few examples of retained and deleted
    console.log("Retained Examples (first 5):");
    retainList.slice(0, 5).forEach(r => {
        console.log(`  - DB Name: "${r.dbContact.full_name}" | Sheet Name: "${r.match.name}" (Email: ${r.dbContact.email}, Phone: ${r.dbContact.phone})`);
    });

    console.log("\nDeleted Examples (first 10):");
    deleteList.slice(0, 10).forEach(d => {
        console.log(`  - DB Name: "${d.full_name}" (Email: ${d.email}, Phone: ${d.phone})`);
    });
}
