                    ...
                    {loading ? (
                        ...
                        <div key={user.id} className="group relative bg-gray-900/40 border border-gray-800 rounded-2xl p-6 hover:border-[#ec028b]/50 transition-all duration-300">
                            ...
                            <div className="mt-4 space-y-2">
                                ...
                                {/* ── Google Calendar Link Button (always visible) ── */}
                                <button
                                    id={`cal-sync-btn-${user.id}`}
                                    onClick={() => openCalendarSync(user)}
                                    className={cn(
                                        "w-full mt-1 flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-200 border",
                                        user.googleCalendarLinked
                                            ...
                                            : "bg-blue-900/10 border-blue-500/20 text-blue-400/70 hover:bg-blue-900/20 hover:border-blue-500/40 hover:text-blue-300"
                                    )}
                                >
                                    {/* Google G logo */}
                                    ...
                                    {user.googleCalendarLinked ? (
                                        <span className="flex items-center gap-1.5">
                                            ...
                                            Calendar Linked
                                            {user.calendarEventCount !== undefined && (
                                                <span className="opacity-60">· {user.calendarEventCount} events</span>
                                            )}
                                        </span>
                                    ) : (
                            ...
                            {calSyncResult?.success ? (
                                ...
                                <>
                                    ...
                                    <div className="bg-gray-800/40 border border-gray-700 rounded-xl px-4 py-3 space-y-1">
                                        <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest">Client ID stored in Firestore</p>
                                        <p className="text-gray-500 text-[10px]">OAuth Client ID is managed at <span className="font-mono text-gray-400">settings / google_oauth</span> in Firestore — no env variables needed.</p>
                                    </div>
