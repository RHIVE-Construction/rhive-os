    ...
    return (
        <div className="space-y-2">
            ...
            <div
                ...
                className={`
                    relative rounded-xl border-2 border-dashed p-6 flex flex-col items-center justify-center gap-3 transition-all duration-200
...
const ToggleSwitch: React.FC<ToggleSwitchProps> = ({ id, checked, onChange, label, disabled }) => (
    <div className="flex items-start gap-4">
        <button
            ...
            className={`
                relative flex-none w-12 h-6 rounded-full border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#ec028b]/40
                ${checked
                    ? 'bg-[#ec028b] border-[#ec028b] shadow-[0_0_12px_rgba(236,2,139,0.4)]'
        ...
        >
            <span
                className={`
                    absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow-md transition-all duration-200
                    ${checked ? 'translate-x-6' : 'translate-x-0'}
                `}
            />
        </button>
        <label htmlFor={id} className={`text-sm leading-relaxed mt-0.5 ${disabled ? 'opacity-50' : 'text-gray-300 cursor-pointer'}`} onClick={() => !disabled && onChange(!checked)}>
            {label}
        </label>
    </div>
);
...
const PaymentTile: React.FC<PaymentTileProps> = ({
    ...
    return (
        <button
            ...
            className={`
                relative w-full text-left p-5 rounded-xl border-2 transition-all duration-200 focus:outline-none
                ${isSelected
                    ? 'border-[#ec028b] bg-[#ec028b]/10 shadow-[0_0_20px_rgba(236,2,139,0.2)]'
        ...
        >
            <div className="flex items-center gap-3 mb-2">
                ...
                <span className={`font-bold text-base ${isSelected ? 'text-[#ec028b]' : 'text-white'}`}>
                    {title}
                </span>
                {isSelected && (
                    <CheckCircleIcon className="w-4 h-4 text-[#ec028b] ml-auto flex-none" />
                )}
            </div>
...
const CustomerSignVerifyPage: React.FC = () => {
    ...
    // Load project from Firestore by token (projectId)
    useEffect(() => {
        if (!token) {
        ...
        const loadProject = async () => {
            try {
                // Try projects collection first, then leads
                let data: ProjectData | null = null;
                const projRef = doc(db, 'projects', token);
                const projSnap = await getDoc(projRef);

                if (projSnap.exists()) {
                    data = { id: projSnap.id, ...projSnap.data() } as ProjectData;
                } else {
                    const leadRef = doc(db, 'leads', token);
                    const leadSnap = await getDoc(leadRef);
                    if (leadSnap.exists()) {
                        data = { id: leadSnap.id, ...leadSnap.data() } as ProjectData;
                    }
                }
                ...
                if (data) {
                    ...
                    setProjectError('Project not found. Please contact RHIVE Construction for assistance.');
                }
            } catch {
    ...
    const isFormValid = policyClaimNumber.trim().length > 0 && agreed && paymentMethod !== null;
                ...
                <div className="relative z-10 max-w-md w-full bg-gray-900/80 border border-gray-800 rounded-2xl p-8 text-center space-y-6 backdrop-blur-sm">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-gray-400 text-sm leading-relaxed">
                            {projectError || 'This link appears to be invalid or has expired.'}
                        </p>
                    </div>
                    <p className="text-gray-500 text-xs">
                        Please contact RHIVE Construction at{' '}
                        <a href="tel:+18014410024" className="text-[#ec028b] hover:underline">
                            (801) 441-0024
                        </a>{' '}
                        for assistance.
                    </p>
                    <button
                        id="csv-error-home-btn"
            ...
            <div className="fixed inset-0 bg-black flex items-center justify-center px-6">
                ...
                <div className="relative z-10 max-w-lg w-full bg-gray-900/80 border border-[#ec028b]/30 rounded-2xl p-10 text-center space-y-6 backdrop-blur-sm shadow-[0_0_40px_rgba(236,2,139,0.1)]">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-gray-400 text-sm leading-relaxed">
                            Your insurance verification information has been submitted to RHIVE Construction.
                    ...
                    <div className="bg-black/40 border border-gray-800 rounded-xl p-4 text-left space-y-2">
                        ...
                        <ul className="text-gray-400 text-xs space-y-1.5">
                            <li className="flex items-start gap-2">
                                <span className="text-[#ec028b] mt-0.5">→</span>
                            ...
                            <li className="flex items-start gap-2">
                                <span className="text-[#ec028b] mt-0.5">→</span>
                                You'll receive a confirmation call or email within 1–2 business days.
                            </li>
                        </ul>
                    </div>
                    <button
                        id="csv-success-home-btn"
                        onClick={handleBackToHome}
                        className="flex items-center gap-2 mx-auto px-6 py-3 bg-[#ec028b]/10 border border-[#ec028b]/40 rounded-xl text-[#ec028b] font-bold hover:bg-[#ec028b]/20 hover:shadow-[0_0_20px_rgba(236,2,139,0.2)] transition-all"
                    >
                        <HomeIcon className="w-4 h-4" />
                        Back to Homepage
                    </button>
        ...
        <div className="fixed inset-0 bg-black overflow-y-auto">
            ...
            <div className="relative z-10 min-h-full py-12 px-4 sm:px-6 flex flex-col items-center">
                ...
                <div className="w-full max-w-2xl mb-6 bg-gray-900/60 border border-gray-800 rounded-2xl p-5 backdrop-blur-sm">
                    <p className="text-[10px] text-gray-600 uppercase font-bold tracking-widest mb-1">
                        Your Project
                    </p>
                    <h2 className="text-white font-extrabold text-lg">{project.name || 'RHIVE Roofing Project'}</h2>
                    {getAddress(project) && (
                        <p className="text-gray-500 text-sm mt-0.5">{getAddress(project)}</p>
                    )}
                    <div className="mt-3 flex items-center gap-2">
                        <span className="text-[10px] text-amber-400 font-bold uppercase tracking-widest px-2 py-0.5 rounded border border-amber-500/30 bg-amber-500/10">
                            Sign &amp; Verify Stage
                        </span>
                ...
                <div className="w-full max-w-2xl mb-6 text-center">
                    ...
                    <p className="text-gray-400 text-sm leading-relaxed max-w-lg mx-auto">
                        Please complete the form below to help us process your insurance claim accurately.
                        Your information is secure and will only be used for your roofing project.
                    </p>
                </div>
                ...
                <div className="w-full max-w-2xl space-y-6">
                    ...
                    <div className="bg-gray-900/60 border border-gray-800 rounded-2xl p-6 backdrop-blur-sm space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                id="csv-claim-number"
                                type="text"
                                value={policyClaimNumber}
                                onChange={(e) => setPolicyClaimNumber(e.target.value)}
                                placeholder="e.g. CLM-2024-00123456"
                                className="w-full bg-black/60 border border-gray-700 rounded-xl px-4 py-3 text-white font-mono placeholder-gray-600 focus:outline-none focus:border-[#ec028b]/60 focus:shadow-[0_0_12px_rgba(236,2,139,0.15)] transition-all"
                                aria-required="true"
                            />
                            <p className="text-gray-600 text-xs">
                                You can find your claim number on your insurance company's claim confirmation letter or email.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-gray-900/60 border border-gray-800 rounded-2xl p-6 backdrop-blur-sm space-y-5">
                        ...
                        <FileUploadZone
                            id="csv-policy-claim-file"
                            label="Insurance Policy Claim Document *"
                            placeholder="Upload your insurance policy claim document. This should be the official documentation from your carrier confirming your claim number, coverage, and scope of loss."
                            file={policyFile}
                        ...
                        <FileUploadZone
                            id="csv-scope-of-work-file"
                            label="Scope of Work or Supporting Documents (Optional)"
                            placeholder="You may also upload a scope of work document here. If your insurance claim document already includes the full scope of loss, you do not need a separate upload."
                            file={scopeFile}
                            onFile={setScopeFile}
                        />
                    </div>
                    ...
                    <div className="bg-gray-900/60 border border-gray-800 rounded-2xl p-6 backdrop-blur-sm space-y-4">
                        ...
                        <p className="text-gray-500 text-xs">
                            Select how you intend to fulfill your portion of the payment. This does not change the scope of work — it helps us align with your insurance arrangement.
                        </p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <PaymentTile
                                id="csv-payment-deductible"
                                value="deductible"
                                selected={paymentMethod}
                    ...
                    <div className="bg-gray-900/60 border border-gray-800 rounded-2xl p-6 backdrop-blur-sm space-y-4">
                        ...
                        <div className="bg-black/40 border border-gray-800 rounded-xl p-4 text-xs text-gray-500 leading-relaxed space-y-2">
                            ...
                            <p>
                                By enabling the toggle below, I hereby confirm and agree that:
                            </p>
                            <ul className="list-disc list-inside space-y-1 pl-2">
                                ...
                                <li>I authorize <strong className="text-gray-300">RHIVE Construction</strong> to act as my authorized representative in matters related to the insurance claim submitted for my roofing project.</li>
                                ...
                                <li>I consent to RHIVE Construction sharing the uploaded documentation with my insurance carrier as necessary to complete the claim process.</li>
                            </ul>
                        </div>

                        <ToggleSwitch
                            id="csv-agreement-toggle"
                            checked={agreed}
                            onChange={setAgreed}
                    ...
                    {!isFormValid && (
                        <div className="flex flex-wrap gap-2">
                            {!policyClaimNumber.trim() && (
                                <span className="text-[10px] text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-full px-3 py-1 font-bold uppercase tracking-widest">
                                    Enter claim number
                                </span>
                            )}
                            {!paymentMethod && (
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 pb-12">
                        <button
                            ...
                            className={`
                                flex-1 flex items-center justify-center gap-3 py-4 px-8 rounded-xl font-black text-base uppercase tracking-wider transition-all duration-200
