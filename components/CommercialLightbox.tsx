        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 text-[var(--rhive-text)]">
            ...
            >
                <div className="flex-shrink-0 p-6 sm:p-8 bg-gradient-to-b from-[#0a0a0a] to-black border-b border-white/5 flex justify-between items-start relative z-10">
                    <div>
                        <div className="inline-flex items-center gap-2 mb-3 bg-[var(--rhive-blue)]/10 border border-[var(--rhive-blue)]/20 px-3 py-1 rounded-sm">
                            ...
                            <span className="text-base uppercase tracking-widest text-[var(--rhive-blue)] font-bold">Compare Options</span>
                        </div>
                ...
                <div className="flex-grow overflow-y-auto p-6 sm:p-8 relative z-10 custom-scrollbar">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {packages.map((pkg, idx) => (
                            ...
                            >
                                {pkg.recommended && (
                                    <div className="absolute top-0 right-0 bg-[var(--rhive-blue)] text-white text-base font-black uppercase tracking-widest px-4 py-1.5 z-20"
                                ...
                                <div className="p-6 border-b border-white/5 relative bg-black/40">
                                    <div className="text-base text-gray-400 font-bold uppercase tracking-widest mb-2">{pkg.tier}</div>
                                    <h3 className="text-2xl font-black text-white uppercase mb-1 font-display">{pkg.name}</h3>
                                    <div className="text-base text-[var(--rhive-blue)] font-bold mb-4">{pkg.system}</div>

                                    <div className="bg-white/5 border border-white/10 p-3 rounded-sm mb-2">
                                        <div className="text-base text-gray-500 uppercase tracking-widest mb-1">Warranty Coverage</div>
                                        <div className="text-base font-bold text-gray-200">{pkg.warranty}</div>
                                    </div>
                                </div>

                                <div className="flex-grow p-6 bg-black/20">
                                    <div className="text-base text-gray-500 uppercase tracking-widest mb-4">Core Specifications</div>
                                    <ul className="space-y-3">
                                        {pkg.highlights.map((highlight, hIdx) => (
                                            <li key={hIdx} className="flex items-start text-base text-gray-300 font-serif group-hover:text-white transition-colors">
                                                <span className={`mr-2 mt-1 text-base ${pkg.recommended ? 'text-[var(--rhive-blue)]' : 'text-gray-500'}`}>■</span>
                                ...
                                <div className="p-6 border-t border-white/5 mt-auto bg-black/60">
                                    <button
                                        ...
                                        className={`w-full py-4 text-base font-black uppercase tracking-widest transition-all duration-300 ${pkg.recommended ? 'bg-[var(--rhive-blue)] text-white hover:bg-white hover:text-black shadow-[0_0_20px_rgba(8,19,124,0.4)]' : 'bg-white/5 text-white hover:bg-white/10 border border-white/10'}`}
                ...
                <div className="flex-shrink-0 p-6 border-t border-white/5 bg-black/80 backdrop-blur-md flex justify-end relative z-10">
                    <button
                        onClick={onClose}
                        className="text-base font-bold uppercase tracking-widest text-gray-400 hover:text-white transition-colors px-6 py-3 border border-transparent hover:border-white/10 bg-transparent hover:bg-white/5"
                        style={{ clipPath: 'polygon(8px 0, 100% 0, 100% calc(100% - 8px), calc(100% - 8px) 100%, 0 100%, 0 8px)' }}
                    >
                        Close Matrix
                    </button>
