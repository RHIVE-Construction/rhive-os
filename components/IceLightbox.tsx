                ...
                <div className="flex-shrink-0 p-6 sm:p-8 bg-gradient-to-b from-[#0a0a0a] to-black border-b border-white/5 flex justify-between items-start relative z-10">
                    <div>
                        <div className="inline-flex items-center gap-2 mb-3 bg-[#0ee]/10 border border-[#0ee]/20 px-3 py-1 rounded-sm">
                            ...
                            <span className="text-base uppercase tracking-widest text-[#0ee] font-bold">Compare Options</span>
                        </div>
                ...
                <div className="flex-grow overflow-y-auto p-6 sm:p-8 relative z-10 custom-scrollbar">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {packages.map((pkg, idx) => (
                            ...
                            >
                                {pkg.recommended && (
                                    <div className="absolute top-0 right-0 bg-[#0ee] text-black text-base font-black uppercase tracking-widest px-4 py-1.5 z-20"
                                ...
                                <div className="p-6 border-b border-white/5 relative bg-black/40">
                                    ...
                                    <div className="text-base text-[#0ee] font-bold mb-4">{pkg.system}</div>
                                    ...
                                    <ul className="space-y-3">
                                        {pkg.highlights.map((highlight, hIdx) => (
                                            <li key={hIdx} className="flex items-start text-base text-gray-300 font-serif group-hover:text-white transition-colors">
                                                <span className={`mr-2 mt-1 text-base ${pkg.recommended ? 'text-[#0ee]' : 'text-gray-500'}`}>■</span>
                                ...
                                <div className="p-6 border-t border-white/5 mt-auto bg-black/60">
                                    <button
                                        ...
                                        className={`w-full py-4 text-base font-black uppercase tracking-widest transition-all duration-300 ${pkg.recommended ? 'bg-[#0ee] text-black hover:bg-white hover:text-black shadow-[0_0_20px_rgba(0,238,238,0.4)]' : 'bg-white/5 text-white hover:bg-white/10 border border-white/10'}`}
